...
for (let i = 0, l = elements.length / ELEMENT_STRIDE; i < l; i += 1) {
  ...
  const $wiki = $element.querySelector('.element-wiki');
  ...
  if ($wiki) {
    const elementName = elements[offset + ELEMENT_FIELDS.NAME];
    $wiki.href = `https://en.wikipedia.org/wiki/${elementName}`;
  }
...
createTimer({
  onUpdate: () => {
    if (isAutoSpinning) {
      autoSpinAngle += 0.005;
      pointer.rx = utils.lerp(pointer.rx, 15, .01);
      pointer.ry = utils.lerp(pointer.ry, 20, .01);
      sceneAnimatable.rotateX(Math.sin(autoSpinAngle) * 5 * pointer.rx);
      sceneAnimatable.rotateY((autoSpinAngle * 10) * pointer.ry);
    } else {
...
const toggles = utils.$('.controls button.toggle:not(.util-btn)');

document.addEventListener('click', event => {
  const $toggle = event.target.closest('.controls button.toggle:not(.util-btn)');
  if ($toggle) {
    ...
    elementsLayout.update(() => {
      ...
      if (layoutType === 'shuffle') {
        $sceneContent.dataset.layout = 'random';
...
const filterCards = () => {
  ...
  const allCards = document.querySelectorAll('#scene-content .element');
  allCards.forEach($card => {
    const $titleEl = $card.querySelector('.element-title');
    const $symbolEl = $card.querySelector('.element-symbol');
    if (!$titleEl || !$symbolEl) return;

    const title = $titleEl.textContent.toLowerCase();
    const symbol = $symbolEl.textContent.toLowerCase();
    
    const matchesSearch = title.includes(term) || symbol.includes(term);
    const matchesCategory = cat === 'all' || $card.dataset.color === cat;
    ...
    if (matchesSearch && matchesCategory) {
      ...
      $card.style.transition = 'opacity 0.4s ease, filter 0.4s';
    } else {
